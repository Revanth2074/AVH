...
def add_job(request):
    ...
    error=""
    if request.method == 'POST':
        jt = request.POST['Ad title']
        sd = request.POST['startdate']
        ed = request.POST['enddate']
        i= request.FILES['Ad image']
        cd = request.POST['Contact details']
        des = request.POST['description']
        user = request.user
        recruiter=Recruiter.objects.get(user=user)
        try:
            Job.objects.create(recruiter=recruiter,start_date=sd,end_date=ed,title=jt,image=i,
                               description=des,experience=cd,creationdate=date.today())
